...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    # Test 4:
    title = 'PIZZA test 4:  200 slices, thin (thickness=3) black lines on a yellow circle.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 3
    circle.fill_color = 'yellow'
    pizza(window, circle, 200, 'blue', 3)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    # Test 3
    title = 'POLYGON tests 3:  4 segments, black lines on lime green circle.'
    window = rg.RoseWindow(550, 400, title)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(number_of_segments):
        line = rg.Line(points[k], points[(k + 1) % len(points)])
...
def run_test_fancy_polygon():
    ...
    # ------------------------------------------------------------------
    # Test 4:
    title = 'FANCY POLYGON test 4:  100 red lines on yellow circle, hops = 50.'
    window = rg.RoseWindow(600, 600, title)

    circle = rg.Circle(rg.Point(300, 300), 250)
    circle.fill_color = 'yellow'
    fancy_polygon(window, circle, 100, 50, 'red', 9)
    window.close_on_mouse_click()
